...
- Terragen 根目录只放克隆的仓库（git 子模组）：
  - `TranslateBooksWithLLMs/`（v1.5.9）：自包含应用，主基座候选——多 provider、checkpoint、glossary、EPUB 处理齐备。
  - `translate-book/`：agent 工作流脚手架，参考其 manifest/run_state 翻译状态追踪设计。
- `Script/`：所有工作文件的交付位置（本计划、para-translation 工具、后续调研与实现产物）。

## 已定结论

### 1. LLM 提供商：DeepSeek 官方为主，硅基流动对照
